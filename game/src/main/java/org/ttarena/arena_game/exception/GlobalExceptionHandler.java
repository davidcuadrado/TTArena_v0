...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleForbidden(ForbiddenException e) {
        return problem(HttpStatus.FORBIDDEN, "not-your-game", "Not allowed", e.getMessage());
    }
    ...
    public ResponseEntity<ProblemDetail> handleUpstreamUnavailable(UpstreamUnavailableException e) {
        log.error("Upstream unavailable: {}", e.getMessage());
        return problem(HttpStatus.SERVICE_UNAVAILABLE, "upstream-unavailable", "A service this game depends on is unavailable", e.getMessage());
    }
    ...
    public ResponseEntity<ProblemDetail> handleConcurrentModification(OptimisticLockingFailureException e) {
        ...
        return problem(HttpStatus.CONFLICT, "concurrent-modification", "Changed by someone else",
                "This game moved on while your turn was being played. Fetch it again and retry.");
    }
